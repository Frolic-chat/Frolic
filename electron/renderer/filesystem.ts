 ...
 */
export function getLogDir(this: unknown, character: string): string {
    const dir = path.join(core.state.generalSettings.logDirectory, character, 'logs');
    fs.mkdirSync(dir, { recursive: true });
    return dir;
}

function getLogFile(this: unknown, character: string, key: string): string {
    return path.join(getLogDir(character), key);
}

export function checkIndex(this: unknown, index: Index, message: Message, key: string, name: string,
...
export function fixLogs(character: string): void {
    ...
    for (const file of files) {
        ...
        if (file.slice(-4) === '.idx') {
            if (!fs.existsSync(full.slice(0, -4)))
                fs.unlinkSync(full);
            continue;
        }
        ...
        try {
            while (pos < size) {
                ...
                if (buffer.readUInt16LE(deserialized.size - 2) !== deserialized.size - 2)
                    throw new Error();
        ...
        if (file.slice(-4) === '.idx') {
            try {
                ...
                const item: IndexItem = {
                    ...
                    index:   {},
                    offsets: new Array<number>(content.length - offset),
                };
...
export class Logs implements Logging {
    ...
    getConversations(character: string): ReadonlyArray<{ key: string, name: string }> {
