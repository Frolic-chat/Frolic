...
<div id="page" style="position:relative; padding:5px 10px 10px" :class="getThemeClass()" @mouseover="onMouseOver" @auxclick.prevent @click.middle="unpinUrlPreview">
    ...
    <div class="card bg-light" style="width: 400px;">
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          ...
          <input id="account" v-model="settings.account" class="form-control" :disabled="loggingIn" @keypress.enter="login()">
        </div>
        <!-- password -->
        <div class="form-group">
          ...
          <input id="password" v-model="password" class="form-control" type="password" :disabled="loggingIn" @keypress.enter="login()">
        </div>
        <div v-show="showAdvanced" class="form-group">
          <label class="control-label" for="host">{{ l('login.host') }}</label>
          <div class="input-group">
            <input id="host" v-model="settings.host" class="form-control" :disabled="loggingIn || !allowedToLogin" @keypress.enter="login()">
        ...
        <div class="form-group">
          <label for="advanced">
            <input id="advanced" v-model="showAdvanced" type="checkbox"> {{ l('login.advanced') }}
          </label>
        </div>
        <!-- save login -->
        <div class="form-group">
          <label for="save">
            <input id="save" v-model="saveLogin" type="checkbox"> {{ l('login.save') }}
          </label>
        </div>
        <!-- login button -->
        <div class="form-group" style="margin:0;text-align:right">
          ...
          <button v-else-if="!fatalError()" class="btn btn-primary" :disabled="loggingIn" @click="login">
  ...
  <chat v-else ref="chat" :ownCharacters="characters" :defaultCharacter="defaultCharacter"></chat>
  <div ref="linkPreview" class="link-preview"></div>
  <modal ref="importModal" :action="l('importer.importing')" :buttons="false">
  ...
  <modal ref="profileViewer" :buttons="false" dialogClass="profile-viewer">
    <character-page ref="characterPage" :authenticated="true" :oldApi="true" :name="profileName"></character-page>
    <template slot="title">
      {{ profileName }}
      ...
      <i v-show="isRefreshingProfile()" class="fas fa-circle-notch fa-spin profileRefreshSpinner"></i>

      <bbcode v-show="!!profileStatus" :text="profileStatus" class="status-text"></bbcode>

      <div class="profile-title-right">
        <button class="btn" :disabled="!prevProfileAvailable()" @click="prevProfile"><i class="fas fa-arrow-left"></i></button>
        <button class="btn" :disabled="!nextProfileAvailable()" @click="nextProfile"><i class="fas fa-arrow-right"></i></button>
      </div>
    </template>
  </modal>
  <modal ref="fixLogsModal" :action="l('fixLogs.action')" buttonClass="btn-danger" @submit="fixLogs">
    <span style="white-space:pre-wrap">{{ l('fixLogs.text') }}</span>
    <div class="form-group">
      <label class="control-label">{{ l('fixLogs.character') }}</label>
      <select id="import" v-model="fixCharacter" class="form-control">
        <option v-for="character in fixCharacters" :key="character" :value="character">
          {{ character }}
        </option>
      </select>
    </div>
...
import type { Task } from './LoginTasks.vue';
import { default as LoginTasks } from './LoginTasks.vue';
...
export default class Index extends Vue {
        ...
        try {
            ...
            core.connection.onEvent('connecting', async() => {
                if (!Electron.ipcRenderer.sendSync('connect', core.connection.character)) {
                    window.alert(l('login.alreadyLoggedIn'));
                ...
                if ((await core.settingsStore.get('settings')) === undefined &&
                    ...
                    if (!window.confirm(l('importer.importGeneral')))
    ...
    fixLogs(): void {
        if (!Electron.ipcRenderer.sendSync('connect', this.fixCharacter))
            return window.alert(l('login.alreadyLoggedIn'));

        try {
            fixLogs(this.fixCharacter);
            window.alert(l('fixLogs.success'));
        }
        catch(e) {
            window.alert(l('fixLogs.error'));
            throw e;
        }
        finally {
    ...
    getThemeClass(): Record<string, boolean> {
        try {
            ...
            const selected_theme = `theme-${core.state.settings.risingCharacterTheme || this.settings.theme}`;
            return {
                [selected_theme]:           true,
    ...
    get styling(): string {
        try {
            ...
            const theme_file = Fs.readFileSync(theme_file_path, 'utf8');
    ...
    start = {
        ...
        listeners: () => {
            ...
            Electron.ipcRenderer.on('fix-logs', () => {
                this.fixCharacters = core.settingsStore.getAvailableCharacters();
