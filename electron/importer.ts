...
export function importGeneral(data: GeneralSettings): void {
    ...
    if (file.slice(-3) === 'xml') {
        ...
        if (elm) elm = elm.firstElementChild;
        if (elm) elm = elm.firstElementChild;
        if (!elm)
            return;
        ...
        for (const element of config) {
            if (!element.firstElementChild || element.firstElementChild.textContent === null)
                continue;
...
function createMessage(line: string, ownCharacter: string, name: string, isChannel: boolean, date: Date): LogMessage | undefined {
    ...
    if (line[lineIndex] === '[') {
        ...
        if(lineIndex + ownCharacter.length <= line.length && line.substring(lineIndex, lineIndex + ownCharacter.length) === ownCharacter) {
            ...
            sender = matched && matched.length > 1 ? matched[1] : '';
        }
...
async function importSettings(dir: string): Promise<void> {
    ...
    function getValue(name: string): string | undefined {
        ...
        return elm?.textContent ?? undefined;
    }
    ...
    for (const element of elements) {
        ...
        if (item !== null && pinned.channels.includes(item)) pinned.channels.push(item);
    }
...
export async function importCharacter(ownCharacter: string, progress: (progress: number) => void): Promise<void> {
    ...
    for (let i = 0; i < subdirs.length; ++i) {
        ...
        if (subdir === '!Notifications' || subdir === 'Global' || !fs.statSync(subdirPath).isDirectory())
            continue;
        ...
        if (channelMarker !== -1) {
            ...
            name = subdir;
            if (knownOfficialChannels.includes(subdir)) {
        ...
        const sortedFiles = files
            .map(filename => {
                ...
                return {
                    name: filename,
                    date: new Date(parseInt(date[2], 10), parseInt(date[0], 10) - 1, parseInt(date[1], 10))
                };
            })
