    ...
    export default class ConversationView extends Vue {
        ...
        async onKeyDown(e: KeyboardEvent): Promise<void> {
            ...
            if(getKey(e) === Keys.Tab) {
                ...
                if (getKey(e) === Keys.ArrowUp && this.conversation.enteredText.trim().length === 0
                    && !e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey) {
                    this.conversation.loadLastSent();
                }
                else if (getKey(e) === Keys.Enter) {
                    // Handles Shift+Enter when "enter to send" is enabled
                    // and solo Enter when "enter to send" is disabled.
