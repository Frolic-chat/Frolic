  ...
  <div v-if="!connected" class="card bg-light" style="width:400px;max-width:100%;margin:0 auto">
    ...
    <div class="card-body">
      ...
      <select v-model="selectedCharacter" class="form-control custom-select">
        <option v-for="character in ownCharacters" :key="character.name" :value="character">
          {{ character.name }}
        </option>
      </select>
...
function copyNode(str: string, node: BBCodeNode, end: Node, range: Range, flags: { endFound?: true }): string {
    ...
    if (node.nextSibling !== null && !flags.endFound) {
        if (node instanceof HTMLElement && window.getComputedStyle(node).display === 'block')
            str += '\r\n';
...
function scanNode(node: BBCodeNode, end: Node, range: Range, flags: { endFound?: true }, hide?: boolean): string {
    ...
    if (node instanceof HTMLElement && window.getComputedStyle(node).display === 'block' && !flags.endFound)
        str += '\r\n';
...
export default class Chat extends Vue {
    ...
    mounted(): void {
        ...
        document.addEventListener('copy', ((e: ClipboardEvent) => {
            ...
            if (start instanceof HTMLElement) {
                ...
                if (!start) {
                    ...
                    start_value = start instanceof HTMLImageElement
                        ? start.alt
        ...
        core.connection.onEvent('closed', (isReconnect) => {
            ...
            if (isReconnect)
                (this.$refs['reconnecting'] as Modal).show(true);

            if (this.connected)
                core.notifications.playSound('logout');
