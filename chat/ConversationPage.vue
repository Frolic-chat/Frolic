...
>
  <template v-slot:prescroll>
    ...
    <template v-if="!isActivity">
      <div class="info d-flex align-items-center">
        ...
        <span class="ml-auto flex-shrink-0"><!-- right side -->
          <span v-if="isChannelMod">
            <a href="#" :aria-label="l('manageChannel.open')" data-balloon-pos="down" class="btn btn-outline-secondary" @click.prevent="showManage()">
      ...
      <div v-show="showSearch" class="search input-group mt-2">
      ...
      <div v-show="isAutopostingAds()" class="auto-ads">
        ...
        <a v-if="adsRequireSetup" class="btn btn-sm btn-outline-primary renew-autoposts" @click="showChannelAds()">
          {{ l('admgr.setup') }}
        </a>
      </div>
    </template>
  ...
  <template v-slot:postscroll>
    ...
    <div v-show="isConsole || isPrivate(conversation) || isChannel(conversation)" style="display: contents;">
      <bbcode-editor ref="mainInput" v-model="conversation.enteredText" style="position:relative;margin-top:5px" :extras="extraButtons"
          ...
          <template v-if="isChannel(conversation) && (conversation.channel.mode === 'both' || conversation.channel.mode === 'chat')">
            <a v-show="conversation.channel.mode === 'both' || conversation.channel.mode === 'chat'" class="btn" :class="{
              ...
              disabled: conversation.channel.mode != 'both' || conversation.adManager.isActive(),
                ...
                href="#" @click.prevent="setSendingAds(false)"
            >
              {{ l('channel.mode.chat') }}
            </a>
          </template>
          <template v-if="isChannel(conversation) && (conversation.channel.mode === 'both' || conversation.channel.mode === 'ads')">
            <a v-show="conversation.channel.mode === 'both' || conversation.channel.mode === 'ads'" class="btn" :class="{
              ...
              disabled: conversation.channel.mode != 'both' || conversation.adManager.isActive(),
                ...
                href="#" @click.prevent="setSendingAds(true)"
            >
              {{ adsMode }}
            </a>
          </template>
...
export default class ConversationView extends Vue {
    ...
    resizeHandler!:   { (e: UIEvent):       unknown };
    keydownHandler!:  { (e: KeyboardEvent): unknown };
    keypressHandler!: { (e: KeyboardEvent): unknown };
    scrollHandler!:   { (e: Event):         unknown };
    ...
    mounted(): void {
        ...
        this.textBox      = this.$refs['mainInput'] as Editor;
        this.layout       = this.$refs['pageLayout'] as HomePageLayout;
    ...
    showChannelAds() { (this.$refs['channelAdSettingsDialog'] as ConversationAdSettings).show() }
    showManage()     { (this.$refs['manageDialog'] as ManageChannel).show() }
