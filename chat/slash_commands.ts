...
export function isAction(this: unknown, text: string): boolean {
    return /^\/me\b/i.test(text);
}

export function isWarn(this: unknown, text: string): boolean {
    return /^\/warn\b/i.test(text);
}

export function isCommand(this: unknown, text: string): boolean {
    return text.charAt(0) === '/' && !isAction(text) && !isWarn(text);
}

export function parse(this: unknown | never, input: string, context: CommandContext): ((this: Conversation) => void) | string {
    const commandEnd = input.indexOf(' ');
    const name = input.substring(1, commandEnd !== -1 ? commandEnd : undefined).toLowerCase();
    const command = Utils.isKeyOfObject(name, commands) && commands[name];
    if (!command)
        return l('commands.unknown');
    ...
    if (command.params) {
        for (let i = 0; i < command.params.length; ++i) {
            ...
            // For status command, do case-insensitive matching
                ...
                else if ((name === 'reloadconfig' || name === 'setmode') && !param.options?.includes(value)) {
                    const arg = i == 0 ? l(`commands.${name}.param${i}`) : i;
                    return l('commands.invalidParam', arg);
                }
                else if (name === 'status' && !param.options?.includes(value)) {
                    const arg = i === 0 || i === 1 ? l(`commands.${name}.param${i}`) : i;
                    return l('commands.invalidParam', arg);
                }
                else if (!param.options?.includes(value)) {
                    return l('commands.invalidParam', value);
                }
                break;
            case ParamType.Number: {
                // Only timeout and gtimeout get here.
                const num = parseInt(value, 10);
                if (isNaN(num)) {
                    let arg: string | number = 0;
                    if (name === 'timeout')
                        arg = i === 1 ? l(`commands.${name}.param${i}`) : i;
                    else if (name === 'gtimeout')
                        arg = i === 1 || i === 2 ? l(`commands.${name}.param${i}`) : i;
                    else
                        arg = i;
...
// const commands = {
    ...
    close: {
        exec:    (conv: PrivateConversation | ChannelConversation) => void conv.close(),
    ...
    timeout: {
        ...
        params:     [
            ...
            { type: ParamType.Number, validator: (x: number) => x >= 1 },
        ],
    },
    gkick: {
        exec: (_, character: string) =>
...
type CommandKeys = 'status' | 'clear' | 'bottle' | 'code' | 'reloadconfig' | 'setmode' | 'me' | 'reward' | 'greports' | 'join' | 'close' | 'priv' | 'uptime' | 'roll' | 'warn' | 'kick' | 'ban' | 'unban' | 'banlist' | 'timeout' | 'gkick' | 'gban' | 'gunban' | 'gtimeout' | 'setowner' | 'ignore' | 'unignore' | 'ignorelist' | 'makeroom' | 'gop' | 'gdeop' | 'op' | 'deop' | 'scop' | 'scdeop' | 'oplist' | 'invite' | 'closeroom' | 'openroom' | 'setdescription' | 'killchannel' | 'createchannel' | 'broadcast' | 'xyzzy' | 'elf';
