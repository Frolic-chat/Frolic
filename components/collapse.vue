...
<div class="modal-content">
  <div class="modal-header input-group flex-nowrap align-items-stretch border-0 p-0" style="cursor:pointer" :class="headerClass" @click="toggle()">
    ...
    <div v-if="$slots.button && $slots.button.length" :style="headerEndStyle" class="input-group-append flex-shrink-0 border-0 header-button-container-container">
      <div :style="headerEndStyle" class="input-group-text btn p-0 border-0 header-button-container"><!-- keep border-left -->
        <a href="#" :class="btnClass" class="btn justify-content-around h-100 d-flex flex-column border-0" :style="buttonEndStyle" @click.prevent="btnClick">
  ...
  <div :style="style" class="collapse-wrapper" style="overflow:hidden"><!-- animation wrapper -->
    <div ref="content" :class="bodyClass" class="modal-body" :style="maxHeight ? `max-height: ${maxHeight}` : ''">
      <slot></slot>
    </div>
  </div>
</div>
    ...
    toggle(state?: boolean, emitSignal: boolean = true, instant: boolean = false) {
        ...
        if (instant) {
            ...
            if (this.collapsed) {
                ...
                this.style.height = `${(this.$refs['content'] as HTMLElement).clientHeight}px`;
