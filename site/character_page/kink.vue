<template>
<div :id="kink.key" class="character-kink" :class="kinkClasses" @click="toggleSubkinks" @mouseover.stop="showTooltip = true" @mouseout.stop="showTooltip = false">
  ...
  <span v-if="((kink.isCustom) && (expandedCustom))" class="kink-custom-desc">{{ kink.description }}</span>
  <template v-if="kink.hasSubkinks">
    <div class="subkink-list" :class="{closed: listClosed}">
      <kink v-for="subkink in kink.subkinks" :key="subkink.id" :kink="subkink" :comparisons="comparisons" :highlights="highlights"></kink>
    </div>
  </template>
  <div v-if="(showTooltip && (!kink.isCustom || !expandedCustom))" class="popover popover-top">
...
export default class KinkView extends Vue {
    ...
    get kinkClasses(): { [key: string]: boolean } {
        ...
        if (!this.kink.isCustom && this.comparisons[theirKinkId] !== undefined) {
