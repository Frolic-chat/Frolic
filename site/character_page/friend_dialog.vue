<template>
<modal ref="modal" :action="'Friends for ' + name" :buttons="false" dialog-class="modal-dialog-centered modal-lg">
  ...
  <template v-if="!loading">
    <div v-if="existing.length" class="card bg-light">
      ...
      <div class="card-body">
        <div v-for="friend in existing" :key="friend.id" class="friend-item">
          ...
          <button type="button" class="btn btn-danger" @click="dissolve(friend)">
            Remove
          </button>
        </div>
      </div>
    ...
    <div v-if="pending.length" class="card bg-light">
      ...
      <div class="card-body">
        <div v-for="request in pending" :key="request.id" class="friend-item">
          ...
          <button type="button" class="btn btn-danger" @click="cancel(request)">
            Cancel
          </button>
        </div>
      </div>
    ...
    <div v-if="incoming.length" class="card bg-light">
      ...
      <div class="card-body">
        <div v-for="request in incoming" :key="request.id" class="friend-item">
          <character-link :character="request.target">
            <img class="character-avatar icon" :src="avatarUrl(request.target.name)">
            {{ request.target.name }}
          </character-link>
          <span class="date">Sent: <date-display :time="request.createdAt"></date-display></span>
          <button type="button" class="btn btn-success acceptFriend" @click="accept(request)">
            Accept
          </button>
          <button type="button" class="btn btn-danger ignoreFriend" @click="ignore(request)">
            Ignore
          </button>
        </div>
      </div>
    ...
    <div class="card bg-light">
      ...
      <div class="card-body">
        <div class="form-inline">
          ...
          <button class="btn btn-secondary" :disable="requesting || !ourCharacter" @click="request">Request</button>
        </div>
      </div>
    </div>
  </template>
