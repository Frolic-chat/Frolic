...
<div class="row character-page" id="pageBody" ref="pageBody">
    <div class="col-12" style="min-height:0">
        <div v-show="loading">
            <div class="alert alert-info">
                {{ l('characterSheet.loading') }}
            </div>
            <div class="card bg-light alert-emulator">
    ...
    <div v-if="profileIsReady" class="col-md-8 col-lg-9 col-xl-10 profile-body">
        <div id="characterView">
            <!-- proffileIsReady checks `this.character` but below can't figure that out so we v-if it on this random div... -->
            <div v-if="character">
                <div id="headerBanReason" v-if="character.ban_reason" class="alert alert-warning">
                    {{ l('characterSheet.banned') }}
                    <br/> {{ character.ban_reason }}

                    <template v-if="character.timeout">
                        <br/>{{ l('characterSheet.timoutTime') }}
                ...
                <div class="card bg-light">
                    <div class="card-header character-card-header">
                        <tabs class="card-header-tabs" v-model="tab">
                            <span>{{ l('characterSheet.tabs.overview') }}</span>
                            <span>{{ l('characterSheet.tabs.info') }}</span>
                            <span v-if="!oldApi">{{ l('characterSheet.tabs.groups', groups ? groups.length : 0) }}</span>
                            <span>{{ l('characterSheet.tabs.images', character.character.image_count || 0) }}</span>
                            <span v-if="character.settings.guestbook">{{ l('characterSheet.tabs.guestbook', guestbook ? guestbook.posts.length : 0) }}</span>
                            <span v-if="character.is_self || character.settings.show_friends">{{ l('characterSheet.tabs.friends', friends ? friends.length : 0) }}</span>
                            <span>{{ l('characterSheet.tabs.recon') }}</span>
                        </tabs>
                    </div>
                    <div class="card-body">
...
/**
 * Things to do:
 * Redo main trigger to fetch character on name-change.
 * Figure out the cache for the page and try to use main cache get instead of a custom get.
 * Move all page information into their own page files.
 * Move all information updates into their own page files(?).
 * Put all parts of pages into a parts directory.
 */
...
const bodyParser = new StandardBBCodeParser();
...
@Component({ components: {
    ...
    bbcode: BBCodeView(bodyParser),
    'bbcode-status': BBCodeView(core.bbCodeParser),
}})
export default class CharacterPage extends Vue {
    /**
     * Character name. For the most part, this is what we have to rely on for looking up the profile, since the character page remains mounted for the whole app lifecycle. So trigger updates off name change, or at least show/hide, instead of vue lifecycle hooks.
     */
    ...
    @Watch('tab')
    switchTabHook(): void {
        const target = <ShowableVueTab | undefined>this.$refs[`tab${this.tab}`];
    ...
    private async refreshCharacter(): Promise<void> {
        ...
        log.debug('refreshCharacter.start', { name: this.name });

        try {
            const character = await methods.characterData(this.name, undefined, false);
...
<style lang="scss">
/* Emulate bootstrap 4 `alert` class */
.alert-emulator {
    margin-bottom: 1rem;
}
